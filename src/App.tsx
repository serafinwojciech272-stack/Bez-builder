...
export default function App(){
 ...
 const [prophetMode,setProphetMode]=useState(false);
 useEffect(()=>{document.body.classList.toggle('prophet-active',prophetMode); return ()=>document.body.classList.remove('prophet-active')},[prophetMode]);
 ...
 return <div className="site">
  <header className="nav"><a className="logo" href="#top"><span className="logoMark">S5</span><span><b>DRONE SHOW</b><small>LAS VEGAS / 28.12.2025</small></span></a><nav><a href="#technology">Technology</a><a href="#show">Show system</a><a href="#simulation">Simulation</a><a href="#documentary">Documentary</a><a href="#facts">Facts</a></nav><button className={'navCta prophetToggle '+(prophetMode?'on':'')} onClick={()=>setProphetMode(v=>!v)}><Zap size={13}/> {prophetMode?'Exit Prophet Mode':'Enter Prophet Mode'}</button></header>

  <main id="top">
   <section className="experienceBar"><span>YOU ARE HERE</span><b>LAS VEGAS · 28 DEC 2025</b><small>TURN SOUND ON · LOOK UP · ENTER THE UPSIDE DOWN</small></section>
   <section className="hero">
    <div className="heroCopy"><div className="kicker"><CircleDot size={11}/> CASE STUDY / AERIAL COMPUTING</div><h1>5,000 drones.<br/><i>One living sky.</i></h1><div className="prophetLine"><span>ELEVEN · PROPHET MODE</span><b>{prophetMode?'THE SIGNAL IS LIVE':'THE PROPHET'}</b><small>{prophetMode?'Observe the sky as a decision system. Every pulse is a state change.':'She sees the signal before the sky does.'}</small></div><p className="heroLead">How the Stranger Things Season 5 Las Vegas spectacle turned storyboards, 3D geometry, swarm choreography and flight operations into a synchronized aerial performance.</p><div className="visitorJourney"><div><span>01</span><b>ARRIVE</b><small>Vegas lights below. The sky is still empty.</small></div><div><span>02</span><b>LOOK UP</b><small>The first signal appears above you.</small></div><div><span>03</span><b>ENTER</b><small>Hawkins becomes real in the night.</small></div></div><div className="heroActions"><button className="primary" onClick={()=>document.getElementById('technology')?.scrollIntoView({behavior:'smooth'})}><span>Explore the system</span><ArrowDown size={15}/></button><button className="textBtn" onClick={()=>setProphetMode(true)}><Zap size={14}/> Enter Prophet Mode</button></div><div className="proof"><div><strong>5,000</strong><span>drones reported by Sky Elements</span></div><div><strong>600 ft</strong><span>Vecna height reported by Sky Elements</span></div><div><strong>~10 min</strong><span>battery window per drone</span></div></div></div>
   ...
   <section className="experience section"><div className="experienceIntro"><div><div className="kicker">THE VISITOR EXPERIENCE</div><h2>You don't watch<br/><i>the show.</i><br/>You enter it.</h2></div><p>The page should behave like the audience experience: anticipation, first signal, recognition, escalation, the Eleven moment, then Vecna. Every interaction reveals another layer of the system.</p></div><div className="experienceRail"><article><span>BEFORE</span><b>THE SKY IS EMPTY</b><p>Noise of the Strip. Hundreds of people waiting. Nothing above but black.</p></article><article><span>FIRST SIGNAL</span><b>ELEVEN</b><p>A single luminous point becomes a pattern. The audience realizes something is happening overhead.</p></article><article><span>ESCALATION</span><b>HAWKINS</b><p>Characters, bikes and the world of the series emerge from thousands of synchronized positions.</p></article><article><span>CLIMAX</span><b>VECNA</b><p>The sky becomes a 600-foot-scale character formation, with the real crowd experiencing it together.</p></article></div></section>
