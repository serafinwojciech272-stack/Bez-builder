...
function render(){
 ...
 <aside class="panel slip"><div class="panel-head"><h2>Twój kupon</h2><p>${picks.length} ${picks.length===1?'typ':'typów'}</p></div><div class="slip-body">${picks.length?picks.map(pickCard).join(''):`<div class="empty">Kupon jest pusty.<br>Wybierz kurs z listy po lewej.</div>`}</div><div class="summary"><div class="line"><span>Kurs łączny</span><strong class="total">${total.toFixed(2)}</strong></div><div class="line"><span>Potencjalna wygrana</span><strong id="payout">${(total*10).toFixed(2)} zł</strong></div><div class="stake"><input id="stake" type="number" min="1" step="1" value="10" aria-label="Stawka"><button class="cta" id="copy" ${picks.length?'':'disabled'}>Kopiuj kupon</button></div><p class="helper">Demo. Brak połączenia z operatorem.</p></div></aside></section></main><div id="toast" class="toast hidden"></div>`;
 ...
 const stake=document.querySelector('#stake');if(stake)stake.oninput=()=>{const value=Math.max(0,Number(stake.value)||0);document.querySelector('#payout').textContent=(total*value).toFixed(2)+' zł'};
}
...
function togglePick(id){const [eventId,market]=id.split('-');const e=events.find(x=>x.id===eventId);const m=e.markets.find(x=>x[0]===market);const exists=picks.some(p=>p.id===id);picks=picks.filter(p=>p.eventId!==eventId);if(!exists)picks.push({id,eventId,name:m[1],odds:m[2],event:`${e.home} vs ${e.away}`});render()}
